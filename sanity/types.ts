...
// Query: *[_type == "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || author->name match $search] | order(_createdAt desc){  _id,   title,   author -> {_id, name, image, bio},   _createdAt,   category,   description,   views,   image,   slug}
